 ...
 * @version $Id: SymbolTable.java 10 2013-04-15 23:26:08Z uqihayes $
...
public class SymbolTable {
     ...
     */
    public SymbolTable() {
        ProcedureEntry predefined = 
            new ProcedureEntry( "<predefined>", Position.NO_POSITION, null );
        currentScope = new Scope( predefined, null, 0 );
        ...
        addConstant("nil", Position.NO_POSITION, Type.NIL_TYPE, 
                StackMachine.NULL_ADDR );
        addOperator("=", Position.NO_POSITION, Type.NIL_RELATIONAL_TYPE );
        addOperator("!=", Position.NO_POSITION, Type.NIL_RELATIONAL_TYPE );
    ...
    /** Enter a new scope.
     * @param name of procedure/function scope is for.
     */
    public Scope newScope( ProcedureEntry entry ) {
        currentScope = new Scope( entry, currentScope, 
                currentScope.getLevel()+1 );
        return currentScope;
    }
    ...
    public void resolveCurrentScope() {
        for( SymEntry entry : currentScope.getEntries() ) {
            entry.resolve();
            // System.out.println( " Resolved " + entry );
        }
    }
    ...
    public SymEntry.ConstantEntry addConstant(String name, Position posn,
        ...
        entry = new SymEntry.ConstantEntry(name, posn, currentScope, type, val);
    ...
    public SymEntry.ConstantEntry addConstant(String name, Position posn,
            ConstExp val) {
        SymEntry.ConstantEntry entry;
        entry = new SymEntry.ConstantEntry(name, posn, currentScope, 
                Type.ERROR_TYPE, val);
        return (SymEntry.ConstantEntry) currentScope.addEntry(entry);
    }
    ...
    public SymEntry.TypeEntry addType(String name, Position posn, Type type) {
        ...
        entry = new SymEntry.TypeEntry(name, posn, currentScope, type);
     ...
     *         already exists in the current scope, in which case return null.
     */
    public SymEntry.VarEntry addVariable(String name, Position posn, Type type){
        SymEntry.VarEntry entry;
        entry = new SymEntry.VarEntry(name, posn, currentScope, type );
    ...
    public SymEntry.ProcedureEntry addProcedure(String name, Position posn) {
        ...
        entry = new SymEntry.ProcedureEntry(name, posn, currentScope );
        return (SymEntry.ProcedureEntry) currentScope.addEntry(entry);
    }
    public SymEntry.ProcedureEntry addProcedure(String name, Position posn,
            Type.ProcedureType type ) {
        SymEntry.ProcedureEntry entry;
        entry = new SymEntry.ProcedureEntry(name, posn, currentScope, type );
        return (SymEntry.ProcedureEntry) currentScope.addEntry(entry);
    }
    ...
    public SymEntry.OperatorEntry addOperator(String name, Position posn,
        ...
        if( entry == null ) {
            ...
            /** Already defined at the current level - extend union type */
            entry.extendType( type );
            return entry;
        } else {
            /** Defined at an outer level create new entry with old
             * union type and extend with new type.
             */
